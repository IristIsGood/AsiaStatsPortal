<template>
  <div>
    ...
    <div v-if="success" class="bg-green-50 text-green-600 text-sm p-3 rounded mb-4">{{ success }}</div>
      ...
      <div v-for="dp in pending" :key="dp.id"
        ...
        <div>
          <p class="text-sm font-medium text-gray-800">
            {{ dp.indicator?.code ?? 'N/A' }} — {{ dp.region?.name ?? 'N/A' }} ({{ dp.year }})
          </p>
          <p class="text-xs text-gray-500">
            Value: <span class="font-medium">{{ dp.value }}</span> &nbsp;|&nbsp; Source: {{ dp.source }}
          </p>
        </div>
...
const form = ref({ code: '', name_en: '', unit: '', category: '' });
const dpForm = ref({ indicator_id: '', region_id: '', year: '', value: '', source: '' });
